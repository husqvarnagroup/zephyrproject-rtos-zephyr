...
static const struct gpio_dt_spec diversity_mode_gpio =
	GPIO_DT_SPEC_GET(DT_NODELABEL(sgrm_antenna), diversity_mode_gpios);
static const struct gpio_dt_spec diversity_state_gpio =
	GPIO_DT_SPEC_GET(DT_NODELABEL(sgrm_antenna), diversity_state_gpios);
static const struct gpio_dt_spec antenna_control_gpio =
	GPIO_DT_SPEC_GET(DT_NODELABEL(sgrm_antenna), antenna_control_gpios);
static const struct gpio_dt_spec antenna_input_gpio =
	GPIO_DT_SPEC_GET(DT_NODELABEL(sgrm_antenna), antenna_input_gpios);
...
static struct led_mirror led_mirrors[] = {
	{
		.source = GPIO_DT_SPEC_GET(DT_NODELABEL(sgrm_antenna), led_red_input_gpios),
		.destination = GPIO_DT_SPEC_GET(DT_NODELABEL(led_red), gpios),
	},
	{
		.source = GPIO_DT_SPEC_GET(DT_NODELABEL(sgrm_antenna), led_green_input_gpios),
		.destination = GPIO_DT_SPEC_GET(DT_NODELABEL(led_green), gpios),
	},
	{
		.source = GPIO_DT_SPEC_GET(DT_NODELABEL(sgrm_antenna), led_blue_input_gpios),
