...
struct factory_data_lv_entry {
	...
	uint8_t data[FACTORY_DATA_TOTAL_LEN_MAX + ALIGNMENT_SAFETY_MARGIN];
};
				...
				struct factory_data_lv_entry *entry)
{
	...
	if (read_size > sizeof(entry->data)) {
		return -ENOMEM;
	}

	ret = flash_area_read(fa, offset_aligned + sizeof(entry->length), &entry->data, read_size);
	if (ret) {
		return ret;
	}
...
static int factory_data_init_lv(void)
{
	int ret;
					      ...
					      off_t offset, void *arg)
{
	...
	name_len = strnlen((const char *)entry->data, CONFIG_FACTORY_DATA_NAME_LEN_MAX);
	if (name_len == 0) {
		/*  */
	...
	return strncmp(name, (const char *)entry->data, name_len) == 0;
}

static bool factory_data_value_exists(const char *const name)
{
	return lv_walk(factory_data_lv.fap, factory_data_value_exists_callback, (char *)name);
}
...
int factory_data_save_one(const char *const name, const void *const value, const size_t val_len)
{
	...
	memcpy(entry.data + name_len + 1, value, val_len);
				      ...
				      void *const arg)
{
	...
	if (entry->data[name_len] != '\0') {
		return -EIO;
	}

	return ctx->user_cb((const char *)entry->data, &entry->data[name_len + 1], value_len,
			    ctx->user_ctx);
}
...
int factory_data_load(const factory_data_load_direct_cb cb, const void *const param)
{
	...
	ret = lv_walk(factory_data_lv.fap, factory_data_load_callback, &ctx);
...
ssize_t factory_data_load_one(const char *const name, uint8_t *const value, const size_t val_len)
{
	...
	ret = ctx.out_buf_size; /* number of bytes read */
